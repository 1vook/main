...
let totalCostWebsite = companies.reduce(
  (prevSum, website) => (prevSum += website.websiteCost),
  0,
)
...
let websiteSince2000And2009 = companies.filter(
  (company) => company.yearOfRelease > 2000 && company.yearOfRelease < 2009,
).length

document.write(
  `кількість сайтів, що було зроблено між 2000р. та 2009р. - ${websiteSince2000And2009} шт. <br>`,
)
...
let investmentBiggest10000 = companies.filter((companies) => {
  let totalInvestment = companies.sponsors.reduce(
...
document.write(
  `кількість сайтів, де сума спонсорських вкладень була більшою за 10000 - ${investmentBiggest10000} шт. <br>`,
)
...
companies.forEach((companies) =>
  companies.sponsors.forEach((sponsor) => allSponsors.push(sponsor)),
)
...
const companyWithBiggestCost = companies.reduce((prevCompany, company) =>
  company.websiteCost > prevCompany.websiteCost ? company : prevCompany,
)
...
const expensiveSites = companies.filter(
  (company) => company.websiteCost > 10000,
)
