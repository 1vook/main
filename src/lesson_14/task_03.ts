...
let priceChange = dataList.filter(
  (dataList) => dataList.price < dataList.old_price,
).length

document.write(
  `Знайти кількість товарів, у яких ціна зменшилась - ${priceChange} шт. <br>`,
)

// 3) Товари, які доступні (sell_status:"available")

let sellStatusAvailable = dataList.filter(
  (dataList) => dataList.sell_status === "available",
)
