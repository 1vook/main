...
function autoNumbersStartedA(autoNumbers: string[]) {
  let startedA = 0
...
function autoNumbersFirstAndLast(autoNumbers: string[]) {
  let a = 0
  for (let i = 0; i < autoNumbers.length; i++) {
...
document.write(
  `Кількість номерів де перша та остання літера співпадає - ${startedNumA} шт <br>`,
)

// складаються з більше ніш 5 символів;

function autoNumbersLength(autoNumbers: string[]) {
  let b = 0
  for (let i = 0; i < autoNumbers.length; i++) {
...
const sumAutoNumbersLength: number = autoNumbersLength(autoNumbers)

document.write(
  `Кількість номерів які складаються з більше ніш 5 символів - ${sumAutoNumbersLength} шт <br>`,
)
