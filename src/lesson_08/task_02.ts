...
function inputNumbersOfVisitors(days: number): number[] {
  // Створюємо масив в який будемо додавати кількість відвідувачів
  let arrNumbers: number[] = []

  //   заповнюємо масив
  for (let i = 0; i < days; i++) {
    let visitorsNumberForDay = parseInt(
      prompt(`Яка кількість відвідувачів була в ${i + 1} день?`, "4")!,
    )
...
const days: number = parseInt(
  prompt(`Введіть скільки днів ми будемо рахувати відвідувачів`, "3")!,
)

// Створюємо константу з нашим масивом
const arrVisitors: number[] = inputNumbersOfVisitors(days)

// -------------------------------------------------------------------
// Виводимо на екран номери днів, протягом яких кількість відвідувачів була меншою за 20;
function outputDaysVisitors20(arrVisitors: number[]) {
  for (let index = 0; index < arrVisitors.length; index++) {
    if (arrVisitors[index] < 20) {
      document.write(
        `У ${index + 1} день було менше 20 відвідувачів, а саме - ${arrVisitors[index]} <br>`,
      )
    }
  }
}
...
// --------------------------------------------------------
// номери днів, коли кількість відвідувачів була мінімальною;
function outputMinVisitors(arrVisitors: number[]) {
...
// --------------------------------------------------------
// номери днів, коли кількість відвідувачів була максимальною
function outputMaxVisitors(arrVisitors: number[]) {
  // Цикл який знаходить мінімальне значення
  let max = -Infinity
  for (let index = 0; index < arrVisitors.length; index++) {
  ...
  for (let i = 0; i < arrVisitors.length; i++) {
    if (arrVisitors[i] === max) {
      document.write(
        `У ${i + 1} день було найбільше відвідувачів - ${max} <br>`,
      )
    }
  }
}
...
let sumWorksDay: number = 0
let sumWeekendDay: number = 0

function weekSum(arrVisitors: number[]) {
  for (let d = 0; d < arrVisitors.length; d++) {
    if (d % 7 < 5) sumWorksDay += arrVisitors[d]
    else sumWeekendDay += arrVisitors[d]
  }
}
