...
function inputAssessments(length) {
    let arr = [];
    for (let i = 1; i <= length; i++) {
        const assessments = parseInt(prompt(`Введіть Вашу оцінку з ${i} предмета`, "2"));
...
function getStudentResult(assessments) {
    let sumResult = 0;
...
function getStudentCategory(assessments) {
    ...
    if (get2)
        document.write("Двійошник");
    else if (get3)
        document.write("Трійошник");
    else if (get4)
...
const numItems = parseInt(prompt(`Скільки предметів ми будемо рахувати?`, "3"));
// --------------------------------------------------------------
// Отримали масив з оцінками
const assessments = inputAssessments(numItems);
// --------------------------------------------------------------
