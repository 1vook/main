...
// 1) Загальну вартість (нові ціни - price)
let priceSum = dataList.reduce((prevSum, item) => (prevSum += item.price), 0);
document.write(`Загальна вартість - ${priceSum} грн. <br>`);
// 2) Знайти кількість товарів, у яких ціна зменшилась (price < old_price).
let priceChange = dataList.filter((dataList) => dataList.price < dataList.old_price).length;
document.write(`Знайти кількість товарів, у яких ціна зменшилась - ${priceChange} шт. <br>`);
// 3) Товари, які доступні (sell_status:"available")
let sellStatusAvailable = dataList.filter((dataList) => dataList.sell_status === "available");
console.log(sellStatusAvailable);
// 4) сформувати новий список об”єктів тільки доступних  для продажу товарів, які міститимуть тільки ідентифікатор товару (id), нову ціну (price), стару ціну (old_price), та ціну у доларах (usd_price)
let availableProducts = dataList
